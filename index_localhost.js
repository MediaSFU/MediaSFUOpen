...
connections.on('connection', async socket => {
  ...
  const convertPolls = (polls) => {
    ...
    polls.forEach((value, key) => {
      let poll = { ...value }
      poll.voters = poll.voters ? Array.from(poll.voters.entries()).reduce((obj, [key, value]) => {
        obj[key] = value;
        return obj;
      }, {}) : {};
  ...
  const updatePollMembers = async ({ poll, polls, roomName, ended = false }) => {
    ...
    try {
      ...
      await rooms[roomName].members.forEach(async member => {
        ...
        try {
          ...
          if (member.islevel !== '2') {
            await member_socket.emit(emitName, { poll: convertPolls([poll])[0], status: ended ? 'ended' : 'started' })
          } else {
            await member_socket.emit(emitName, { poll: convertPolls([poll])[0], polls: convertPolls(polls), status: ended ? 'ended' : 'started' })
          }
        } catch (error) {
  ...
  const validateRooms = ({ breakoutRooms, roomName }) => {
    ...
    mode == 'sandbox' ? itemPageLimit = meetingRoomParams_Sandbox.itemPageLimit : itemPageLimit = meetingRoomParams_Production.itemPageLimit
    ...
    for (let room of breakoutRooms) {
      ...
      for (let participant of room) {
        ...
        if (typeof participant.name !== 'string' || participant.name.length < 2 || participant.name.length > 10) {
          return false;
        }

        if (typeof participant.breakRoom != 'number' || participant.breakRoom < 0 || participant.breakRoom > 500) {
          return false;
        }
      }
  ...
  const checkBreakoutRooms = ({ roomName, breakoutRooms, newParticipantAction }) => {
    try {
      ...
      if (newParticipantAction != 'autoAssignNewRoom' && newParticipantAction != 'autoAssignAvailableRoom' && newParticipantAction != 'manualAssign') {
      ...
      if (validateRooms({ breakoutRooms, roomName: roomName })) {
        ...
        rooms[roomName].members.forEach(member => {
          const room = breakoutRooms.find(r => r.find(p => p.name === member.name));
          if (!room) {
  ...
  const assignBreakRoomParticipant = ({ name, roomName }) => {
    ...
    try {
      ...
      if (member.breakRoom !== null) {
        ...
        if (tempEventRooms[roomName] && tempEventRooms[roomName].tempBreakoutRooms) {
          const tempBreakoutRooms = tempEventRooms[roomName].tempBreakoutRooms;
          const tempMember = tempBreakoutRooms.find(r => r.find(p => p.name === name));
          if (tempMember) {
      ...
      if (member.breakRoom !== null) {
        return { success: false, reason: 'Participant already assigned to a breakout room' };
      }
      ...
      if (newParticipantAction === 'autoAssignNewRoom') {
        ...
        if (breakoutRooms.length < 500) {
          ...
          rooms[roomName].breakoutRooms = breakoutRooms;
          rooms[roomName].members = rooms[roomName].members.map(m => m.id === member.id ? member : m);
          ...
          return { success: false, reason: 'Maximum number of breakout rooms reached' };
        }
  ...
  const removeBreakRoomParticipant = ({ name, roomName }) => {
    try {
      ...
      if (member && member.breakRoom !== null) {
        ...
        rooms[roomName].breakoutRooms = breakoutRooms;
        rooms[roomName].members = members.map(m => m.id === member.id ? member : m);
        ...
        return { success: false, reason: 'Participant not found in any breakout room' };
      }
    } catch (error) {
      return { success: false, reason: 'An error occurred' };
    }
  ...
  const updateBreakoutMembers = async ({ roomName, ended = false, forHost = false }) => {
    ...
    try {
      ...
      await rooms[roomName].members.forEach(async member => {
        ...
        try {
          let member_socket = peers[member.id].socket
          if (member.islevel != '2') {
            await member_socket.emit(emitName, { breakoutRooms: rooms[roomName].breakoutRooms, status: ended ? 'ended' : 'started', newRoom: hostNewRoom, forHost })
          } else {
            await member_socket.emit(emitName, { breakoutRooms: rooms[roomName].breakoutRooms, status: ended ? 'ended' : 'started', members: rooms[roomName].members, newRoom: hostNewRoom, forHost })
          }
        } catch (error) {
  ...
  const validateWhiteboardUsers = ({ roomName, whiteboardUsers }) => {
    ...
    if (whiteboardUsers.length > itemPageLimit) {
      return { success: false, reason: 'Invalid whiteboard users; too many users' };
    }
    ...
    for (let participant of whiteboardUsers) {
      ...
      if (typeof participant.useBoard !== 'boolean' || participant.useBoard === undefined) {
        return false;
      }
    }
    ...
    rooms[roomName].members.forEach(member => {
      ...
      if (!boarded && member.islevel != '2') {
        ...
        member.useBoard = member.islevel == '2' ? true : boarded ? boarded.useBoard : false
      }
    })
    ...
    try {
      ...
      await rooms[roomName].members.forEach(async member => {
        ...
        try {
          let member_socket = peers[member.id].socket
          if (member.islevel != '2') {
            await member_socket.emit(emitName, { whiteboardUsers: rooms[roomName].whiteboardUsers, status: ended ? 'ended' : 'started' })
          } else {
            await member_socket.emit(emitName, { whiteboardUsers: rooms[roomName].whiteboardUsers, status: ended ? 'ended' : 'started', members: rooms[roomName].members })
          }
        } catch (error) {
  ...
  const assignWhiteBoardParticipant = ({ name, roomName }) => {
    ...
    try {
      ...
      if (member.useBoard) {
        return { success: false, reason: 'Participant already assigned to a whiteboard room' };
      }

      if (tempEventRooms[roomName] && tempEventRooms[roomName].tempWhiteboardUsers) {
        const tempWhiteboardUsers = tempEventRooms[roomName].tempWhiteboardUsers;
        const tempMember = tempWhiteboardUsers.find(r => r.name === name && r.useBoard === true);
        if (tempMember) {
      ...
      if (!member.useBoard) {
        return { success: false, reason: 'Participant not found in any whiteboard room' };
      }
  ...
  const updateWhiteboardAction = async ({ roomName, action, payload, name }) => {
    try {
      const emitName = 'whiteboardAction';

      await Promise.all(rooms[roomName].members.map(async member => {
        try {
          let member_socket = peers[member.id].socket;
          if (member.name !== name) {
    ...
    try {
      ...
      if (roomName) {
        ...
        if (breakoutRoomStarted && !breakoutRoomEnded) {
          try {
            tempEventRooms[roomName].tempBreakoutRooms = rooms[roomName].breakoutRooms
  ...
  socket.on('joinRoom', async ({ roomName, islevel, member }, callback) => {
    ...
    try {
      ...
      if (rooms[roomName].currentPollId && rooms[roomName].roomPolls && rooms[roomName].roomPolls.length > 0) {
        let poll = rooms[roomName].roomPolls.find(poll => poll.id == rooms[roomName].currentPoll.id)
        let polls = rooms[roomName].roomPolls

        if (islevel == '2') {
          await socket.emit('pollUpdated', { poll: convertPolls([poll])[0], polls: convertPolls(polls), status: 'started' })
        } else {
          await socket.emit('pollUpdated', { poll: convertPolls([poll])[0], status: 'started' })
        }
      }
        ...
        if (members) {
          ...
          if (member_Index === -1) {
            ...
            members = await [...members, {
              ...
              requests: [], videoID: '', ScreenID: '', audioID: '', breakRoom: null, useBoard: islevel === '2' ? true : false
            }]
      ...
      try {
        if (rooms[roomName].breakoutRoomStarted && !rooms[roomName].breakoutRoomEnded) {
          if (islevel != '2') {
            await assignBreakRoomParticipant({ name: member, roomName })
          }
          await socket.emit('breakoutRoomUpdated', { breakoutRooms: rooms[roomName].breakoutRooms, status: 'started' })
        }
      } catch (error) {
      ...
      try {
        ...
        if (rooms[roomName].whiteboardStarted && !rooms[roomName].whiteboardEnded) {
          if (islevel != '2') {
            await assignWhiteBoardParticipant({ name: member, roomName })
          }
          await socket.emit('whiteboardUpdated', { whiteboardUsers: rooms[roomName].whiteboardUsers, status: 'started', whiteboardData: rooms[roomName].whiteboardData })
        } else {

          if (islevel != '2') {
            await assignWhiteBoardParticipant({ name: member, roomName })
          }
          if (rooms[roomName].whiteboardData && Object.keys(rooms[roomName].whiteboardData).length > 0) {
            await socket.emit('whiteboardUpdated', { whiteboardUsers: rooms[roomName].whiteboardUsers, status: 'ended', whiteboardData: rooms[roomName].whiteboardData })
          }
        }
      } catch (error) {
  ...
  socket.on('createPoll', ({ roomName, poll }, callback) => {
    ...
    try {
      ...
      if (!poll.question || poll.question.length > 300 || !poll.options || poll.options.length < 2 || poll.options.length > 5) {
      ...
      if (rooms[roomName].roomPolls.length >= 15) {
        return callback({ success: false, reason: 'Maximum number of polls reached' });
      }

      let socketId = socket.id
      let islevel = rooms[roomName].members.find(member => member.id === socketId).islevel

      if (islevel === '2' || islevel === 2) {
      } else {
        callback({ success: false, reason: 'You are not allowed to start a poll' });
      }
  ...
  socket.on('votePoll', async ({ roomName, poll_id, member, choice }, callback) => {
    ...
    try {
      ...
      if (poll && poll.status === 'active') {
        ...
        socket.emit('pollUpdated', { poll: convertPolls([poll])[0], status: 'voted' });


        const roomHost = rooms[roomName].members.find(member => member.isHost);

        if (roomHost) {
          const host_socket = peers[roomHost.id].socket;
          host_socket.emit('pollUpdated', { poll: convertPolls([poll])[0], polls: convertPolls(polls), status: 'voted' });
        }
      } else {
        callback({ success: false, reason: 'Poll does not exist or is not active' });
      }
    } catch (error) {
  ...
  socket.on('startBreakout', async ({ breakoutRooms, newParticipantAction, roomName }, callback) => {
    ...
    try {
      ...
      if (rooms[roomName].breakoutRoomStarted && !rooms[roomName].breakoutRoomEnded) {
        return callback({ success: false, reason: 'Breakout rooms already started' });
      }

      const checkBreak = checkBreakoutRooms({ breakoutRooms, roomName: roomName, newParticipantAction });
  ...
  socket.on('updateHostBreakout', async ({ roomName, prevRoom, newRoom }, callback) => {
    ...
    try {
      ...
      if (!rooms[roomName].breakoutRoomStarted) {
        return callback({ success: false, reason: 'Breakout rooms not started' });
      }
      ...
      if ((prevRoom == undefined || prevRoom == null) || (newRoom == undefined || newRoom == null)) {
  ...
  socket.on('updateBreakout', async ({ breakoutRooms, newParticipantAction, roomName }, callback) => {
    ...
    try {
      ...
      if (!newParticipantAction) {
        if (!rooms[roomName].breakoutRoomAction) {
          return callback({ success: false, reason: 'New participant action not defined' });
        } else {
  ...
  socket.on('startWhiteboard', async ({ roomName, whiteboardUsers }, callback) => {
    try {
      ...
      if (!whiteboardUsers) {
        return callback({ success: false, reason: 'Whiteboard users not defined' });
      }
  ...
  socket.on('updateWhiteboard', async ({ roomName, whiteboardUsers }, callback) => {
    try {
  ...
  socket.on('updateBoardAction', async (data, callback) => {
    try {
      ...
      if (size && payload && JSON.stringify(payload).length > size) {
        return callback({ success: false, reason: `Payload size exceeds limit of ${size / 1024} KB` });
      }

      if (type && typeof payload !== type) {
        return callback({ success: false, reason: `Invalid payload type. Expected ${type}` });
      }

      if (validActions[action].size && !payload) {
        return callback({ success: false, reason: 'Missing payload for action' });
      }
      ...
      if (rooms[roomName]) {
        ...
        if (!room.whiteboardStarted || room.whiteboardEnded) {
          return callback({ success: false, reason: 'Whiteboard not started or has ended' });
        }


        const whiteboardUsers = await room.whiteboardUsers;
        const name = await room.members.find(member => member.id == socket.id).name;
        ...
        if (!user && name !== host.name) {
          return callback({ success: false, reason: 'User not part of the whiteboard users' });
        }
        ...
        whiteboardData.useImageBackground = whiteboardData.useImageBackground !== undefined ? whiteboardData.useImageBackground : true;
        ...
        switch (action) {
          case 'draw':
            if (payload.type === 'freehand') {
              whiteboardData.shapes.push({ type: 'freehand', points: payload.points, color: payload.color, thickness: payload.thickness });
            } else {
              whiteboardData.shapes.push({ type: 'line', x1: payload.x1, y1: payload.y1, x2: payload.x2, y2: payload.y2, color: payload.color, thickness: payload.thickness, lineType: payload.lineType });
            }
            break;
          case 'shape':
            whiteboardData.shapes.push({ type: payload.type, x1: payload.x1, y1: payload.y1, x2: payload.x2, y2: payload.y2, color: payload.color, thickness: payload.thickness, lineType: payload.lineType });
            break;
          case 'erase':
          ...
          case 'clear':
            ...
            break;
          case 'uploadImage':
            whiteboardData.shapes.push({ type: 'image', src: payload.src, x1: payload.x1, y1: payload.y1, x2: payload.x2, y2: payload.y2 });
            break;
          case 'toggleBackground':
            whiteboardData.useImageBackground = !whiteboardData.useImageBackground;
            break;
          case 'undo':
          ...
          case 'redo':
            ...
            break;
          case 'text':
            whiteboardData.shapes.push({ type: 'text', text: payload.text, x: payload.x, y: payload.y, color: payload.color, font: payload.font, fontSize: payload.fontSize });
            break;
          case 'deleteShape':
            whiteboardData.shapes = whiteboardData.shapes.filter(shape => shape !== payload);
            break;
          case 'shapes':
